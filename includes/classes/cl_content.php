...
class content {
	...
	public static function printOverview($language = NULL, $container_id = 'contentoverview') {
		...
		while($data = db_fetch_array($content_query)) {
			$return_string = $return_string . '<tr id="'. $data['content_id'] .'_'. $data['language_id'] .'"><td name="title">'. $data['title'] .'</td><td><a href="#" id="edit_content" rel="'. $data['content_id'] .'_'. $data['language_id'] .'">editicon</a></td><td><a href="#" id="delete_content" rel="'. $data['content_id'] .'_'. $data['language_id'] .'">deleteicon</td></tr>';
		}
