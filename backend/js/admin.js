...
$(function() {
	...
	$("body").on("click",
			"form[id=loginformbackend] input[type=submit][id=login]",
			function() {
	...
	$("body").on("click", "a[id=myshop]", function() {
		...
		$.ajax({
			...
			data : {
				action : "get_customizing_overview"
			}
		}).done(function(msg) {
			$('.content').html(msg);
	...
	$("body")
			.on(
					"change",
					"select[id=product_edit_language_selection]",
					(function() {
						var language_id = $(
								'select[id=product_edit_language_selection] option:selected')
								.attr('id');
						var product_id = $('input[type=hidden][id=product_id]')
								.val();
						$.ajax({
							type : "POST",
							url : "logic/process_action.php",
	...
	$("body").on("change", "select[id=language_selection]", (function() {
		/*
		 * var language_id = $('select[id=language_selection]
		 * option:selected').attr('id'); alert('trigger me'); $.ajax({ type:
		 * "POST", url: "logic/process_action.php", data: { action:
		 * "open_product_editor", product_id: product_id , language_id:
		 * language_id} }).done(function( msg ) { $('.content').html( msg ); });
		 * 
		 */
	...
	$("body").on(
			"click",
			"input[type=submit][id=submit_edit_product]",
			function() {
				...
				var contract_periode = $(
						'input[type=text][id=contract_periode]').val();
	...
	$("body")
			.on(
					"click",
					"input[type=submit][id=give_prod_new_attr]",
					function() {

						var product_id = $('input[type=hidden][id=product_id]')
								.val();
						var language_id = $(
								'input[type=hidden][id=language_id]').val();
								...
								.ajax(
										{
											...
											url : "logic/process_action.php",
											data : {
												action : "open_create_new_attribute_for_product",
												product_id : product_id,
												language_id : language_id
											}
										}).done(function(msg) {
	...
	$("body").on(
			"click",
			"input[type=submit][id=save_new_attr_for_prod]",
			function() {
				...
				$.ajax({
					...
					url : "logic/process_action.php",
					data : {
						action : "create_new_product_info",
						product_id : product_id,
	...
	$("body")
			.on(
					"click",
					"input[type=submit][id=submit_edit_attributes]",
					function() {

						var product_id = $('input[type=hidden][id=product_id]')
								.val();
						var attr_arrayFromPhp = $(
								'input[type=hidden][id=attr_array]').val();
						...
						$.ajax({
							...
							url : "logic/process_action.php",
							data : {
								action : "update_attributes_in_prod_info",
								product_id : product_id,
								joined_attr_id_array : joined_attr_id_array,
								joined_value_array : joined_value_array
							}
						}).done(function(msg) {
							$('.content').html(msg);
						});
	...
	$("body").on("click", "a[id=translate_product]", function() {
		...
		$.ajax({
			...
			url : "logic/process_action.php",
			data : {
				action : "open_translate_product_form",
				product_id : product_id,
				language_id : language_id
			}
		}).done(function(msg) {
	...
	$("body").on(
			"click",
			"input[type=submit][id=submit_translate_product]",
			function() {
	...
	$('body').on("click", "a[id=create_new_product]", function() {
		...
		$.ajax({
			...
			url : "logic/process_action.php",
			data : {
				action : "open_create_product_form"
			}
		}).done(function(msg) {
			$('.content').html(msg);
		});
	...
	$("body").on(
			"click",
			"input[type=submit][id=save_product]",
			function() {
	...
	$("body").on("click", "a[id=myproductattributes]", function() {
		...
		$.ajax({
			...
			url : "logic/process_action.php",
			data : {
				action : "get_product_attributes_overview"
			}
		}).done(function(msg) {
			$('.content').html(msg);
		});
	...
	$("body").on("click", "a[id=create_new_product_attribute]", function() {
		...
		$.ajax({
			...
			url : "logic/process_action.php",
			data : {
				action : "open_create_attribute_form"
			}
		}).done(function(msg) {
			$('.content').html(msg);
		});
	...
	// open form to translate product attributes
	$("body").on("click", "a[id=translate_product_atrribute]", function() {
		var primaryKeysFromPhp = $(this).attr('rel');
		var primaryKeys = primaryKeysFromPhp.split(",");
		...
		$.ajax({
			...
			url : "logic/process_action.php",
			data : {
				action : "open_translate_attribute_form",
	...
	$("body").on(
			"click",
			"input[type=submit][id=submit_translate_product_attribute]",
			function() {
	...
	// Open Product attribute Editor for product with the language which was
	// choosen from the select box
	$("body")
			.on(
					"change",
					"select[id=attribute_edit_language_selection]",
					(function() {
						var language_id = $(
								'select[id=attribute_edit_language_selection] option:selected')
								.attr('id');
						var product_attribute_id = $(
								'input[type=hidden][id=product_attribute_id]')
								.val();
						$.ajax({
							type : "POST",
							url : "logic/process_action.php",
	...
	$('body').on("click", "a[id=create_new_server]", function() {
		...
		$.ajax({
			...
			url : "logic/process_action.php",
			data : {
				action : "open_create_server_form"
			}
		}).done(function(msg) {
			$('.content').html(msg);
		});
	...
	$("body").on(
			"click",
			"input[type=submit][id=create_server]",
			function() {
				...
				var froxlor_username = $(
						'input[type=text][id=froxlor_username]').val();
				var froxlor_password = $(
						'input[type=password][id=froxlor_password]').val();
				var froxlor_db = $('input[type=text][id=froxlor_db]').val();
				var froxlor_db_host = $('input[type=text][id=froxlor_db_host]')
						.val();
				...
				$.ajax({
					...
					url : "logic/process_action.php",
					data : {
						action : "create_new_server",
						name : name,
						mngmnt_ui : mngmnt_ui,
						ipv4 : ipv4,
						ipv6 : ipv6,
						froxlor_username : froxlor_username,
						froxlor_password : froxlor_password,
			...
			.on(
					...
					function() {
						var server_id = $('input[type=hidden][id=server_id]')
								.val();
						var name = $('input[type=text][id=name]').val();
						var mngmnt_ui = $('input[type=text][id=mngmnt_ui]')
								.val();
						var ipv4 = $('input[type=text][id=ipv4]').val();
						var ipv6 = $('input[type=text][id=ipv6]').val();
						var froxlor_username = $(
								'input[type=text][id=froxlor_username]').val();
						var froxlor_password = $(
								'input[type=password][id=froxlor_password]')
								.val();
						var froxlor_db = $('input[type=text][id=froxlor_db]')
								.val();
						var froxlor_db_host = $(
								'input[type=text][id=froxlor_db_host]').val();
						var total_space = $('input[type=text][id=total_space]')
								.val();
						var free_space = $('input[type=text][id=free_space]')
								.val();

						var active;
						if ($('input[type=checkbox][id=active]')
	...
	$("body").on("click", "a[id=create_new_content]", function() {
		...
		$.ajax({
			...
			url : "logic/process_action.php",
			data : {
				action : "open_new_content_editor"
			}
		}).done(function(msg) {
			$('.content').html(msg);
			initCKEditor();
	...
	$("body").on(
			"click",
			"input[type=submit][id=create_content]",
			function() {
	...
	$("body").on(
			"click",
			"a[id=change_invoice_status]",
			function() {

				var invoice_id = $(this).attr('rel');
				var status_id = $(
						'select[name=statusbox_' + invoice_id
	...
	// get page to change admin password
	$("body").on("click", "a[id=mypassword]", function() {
	...
	// update admin password on DB
	$("body").on(
			"click",
			"input[type=submit][id=change_admin_password]",
			function() {

				var new_password = $(
						'input[type=password][id=new_admin_password]').val();

				// just update password if input is greater than 1
				if (new_password != '') {
					$.ajax({
						type : "POST",
						url : "logic/process_action.php",
 ...
 * 
 */
function showMessagePopup(msgtype, msg) {
